...
- **版本管理**: 已定格至 **V0.30**（🔒），当前工作版本 **V0.31**（草稿）
...
| **V0.30** | 🔒 定格（2026-08-18） | 地图模拟大量增强：部署干员/索敌攻击/敌攻范围/积雪机制/时间轴打点/字体本地化等 |
| **V0.31** | 草稿（当前） | 3D 地图页 UI/画面表现优化，统一全站风格 |

**同步流程**：
1. 主目录 (`E:\WebProjects\DGWEB`) 修改
2. 同步到 `E:\WebProjects\versions\DGWEB\DGWEB_V0.31\`
...
- 【2026-08-18 场景与模型同步显示】加载关卡时等待敌人/道具 Spine 首次加载完成后再隐藏 Loading，避免场景先出、模型后弹入
- 【2026-08-18 加载百分比进度条】Loading 增加百分比进度条，场景/模型未就绪前保持不透明遮挡，就绪后一起显示
- 【2026-08-18 修复加载遮罩覆盖】修复加载遮罩被后续样式覆盖导致半透明/层级不足的问题，确保加载完成前完全遮挡地图
- 【2026-08-18 去掉圆形加载动画】Loading 仅保留文字与百分比进度条
- 【2026-08-18 干员部署移至右侧 Tab】干员部署面板移至右侧 Tab「部署」，干员列表改为纵向排列
- 【2026-08-18 部署干员头像本地化】317 张可用头像下载到 images/ops 本地，缺失头像用稀有度色块占位，不再依赖远程头像加载
...
- 【2026-08-15 全屏压扁修复】根因：全屏中打开部署面板等动态内容压缩地图容器高度，画布缓冲不更新被 CSS 拉伸压扁；新增 applyCanvasSize() + ResizeObserver 监听 #mapWrap，容器尺寸变化实时同步缓冲与 aspect；本地+线上 v20260815i 验证
- 【2026-08-15 干员模型尺寸与动画修复】模型缩放 0.9→1.25（约 1 格高）；动画不动根因：干员 Default 动画为时长 0 静态姿势，改为优先选有时长待机动画（Relax/Idle/Move/Battle/Interact）且暂停时也持续播放；本地+线上 v20260815j 验证（银灰 Relax 动画持续变化）
- 【2026-08-15 干员/敌人互相索敌攻击】干员按当前攻击范围（模板+朝向旋转）索敌最近敌人、切攻击动作（干员无 Attack 用 Interact/Move 兜底）并面朝目标；敌人按官方 rangeRadius 索敌（近战 1 格）切 Attack 动画；纯演示不造成伤害（不改血量/击杀）；本地+线上 v20260815k 验证
- 【2026-08-15 敌人攻击范围显示】新增「敌攻范围」开关：点击敌人单独显示攻击范围 + 命中干员红圈 + 详情卡文案；实现对齐参考站（Enemy.ts）——远程敌人（applyWay=RANGED/ALL）画红色半径圆环（半径=rangeRadius−0.25、贴地、renderOrder=100、随敌移动），近战不画范围仅命中标记；enemy_index 补入官方 applyWay（2130 全量）；本地+线上 v20260815l/m 验证
- 【2026-08-15 敌人攻击严格范围】敌人攻击判定收紧为与显示圆环一致（远程=rangeRadius−0.25 有效半径、近战=相邻 1 格），圈外干员不再被攻击；统一 attack/显示/命中三处口径；本地+线上 v20260815n 验证（53 例范围内正常、12 例圈外不再误伤、0 违例）
- 【2026-08-15 字体本地化】全站 30+ 页面 Google Fonts（fonts.gstatic woff2）在国内 404；新增 fonts/fonts.css + SpaceGrotesk.woff2（google/fonts TTF 转 woff2 48KB），中文改系统字体栈，31 文件批量替换彻底移除 Google Fonts 依赖；本地+线上 v20260815o 验证无 404
- 【2026-08-15 敌人攻击严格范围实装（修复近战远处误攻）】用户反馈近战仍在攻击很远处的干员；根因：同 id 敌人共享一个 Spine 骨架与画布，updateCombat 中「同 id 任一在攻击 → 整个 id 播 Attack」使远处同 id 敌人也显示攻击动作；修复：每个敌人 id 新增独立 Attack 骨架/画布（仅该 id 有敌人在攻击时渲染），每个敌人按自己的 attacking 状态在移动/攻击贴图间切换，远处敌人保持移动/待机不再误播攻击；敌人索敌判定与显示圆环/命中标记统一走 enemyRangeCells（近战=相邻 4 格、远程=官方 rangeRadius−0.25 有效半径内格），attack/显示/命中三处口径一致；本地+线上验证：camp_01 同 id 史莱姆 t=10 时 6.08 格处敌人不显示攻击画面、1 格内敌人正常攻击（0 违例）；线上真实 Spine 下 4 类近战敌人 81 次可见检查 attacking==showsAttack 0 违例、独立 Attack 画布正常渲染；版本标记 v20260815o→p，commit cb3aff08
- 【2026-08-15 信件机制敌人不直接攻击（惊艳事热衷者/美食的奴隶）】用户反馈 TO-EX-2 这两个敌人仍会打到干员；PRTS 机制核对：惊艳事热衷者=首次攻击在目标所在格投放<惊喜信件>，美食的奴隶=吞食<惊喜信件>/信件失效时爆炸，均不直接攻击干员（官方 enemy_database 的 applyWay 虽为 MELEE，但实际是信件机制）；修复：map-sim 新增 NO_DIRECT_ATTACK_IDS（好外号热衷者/惊艳事热衷者/“美食的诱惑”/“美食的奴隶”4 个 id），这些敌人 enemyRangeCells 返回空 → 不显示命中范围/红圈、不触发攻击动画，敌攻范围详情卡标注「信件机制（不直接攻击）」；验证：TO-EX-2 贴脸（d=0/1）attacking 恒 false、普通近战「戴面具的孩童」对照照常攻击（20 次、min_d=1）、远处同 id 不误攻、无 JS 报错；版本标记 v20260815p→q
- 【2026-08-15 近战攻击改为「被阻挡才攻击阻挡者」】用户反馈「寻路者战士也在打初雪」，询问是否应写成近战被阻挡才攻击阻挡它的单位；确认为真实游戏语义，已实现：新增 meleeBlocker——近战（地面）敌人只攻击其行进方向下一格/当前格的干员（干员必须放在敌人路线上才会被攻击，路过旁边的干员不打）；飞行近战不受阻挡（保持相邻格判定），远程逻辑不变；敌攻范围命中显示同步为只标阻挡干员，详情卡文案改「近战 · 被阻挡时攻击阻挡者」；验证：camp_01 史莱姆——干员放路线上→接近（d=1）与重叠（d=0）时攻击（命中格=1）、干员放路线旁（距离 1 格路过）→0 次攻击；版本标记 v20260815q→r
- 【2026-08-15 圣聆初雪 天赋+二技能实装 + 干员技能开关】用户要求实装圣聆初雪（6★术师 char_1046_sbell2）二技能与天赋并新增「开启选中技能」选项：天赋1「无垢的雪景」=攻击范围内地面每10秒+1层积雪（每层使经过敌人移速-10%、最多5层）；二技能「霜涛覆岭」=开启后范围内积雪每2秒额外+1层、超5层向周围扩散（最多20格）、离开积雪获得5秒寒冷（移速-15%）；干员菜单新增「开启技能/关闭技能」按钮（作用于当前选中的技能，开启后底座环变蓝提示）；积雪以蓝色半透明地面+「雪N/冻」标签显示；实现为确定性模型（由部署/技能开关时间推导，拖拽回看一致）；模拟器无HP/伤害系统，雪上每秒20%攻击力法术伤害以减速/冻结表现；天赋2「圣山的祝福」（受击触发）暂不生效
  - 【2026-08-15 修正】冻结仅作用于蓝门（保护目标点）：积雪在蓝门格达5层时蓝门冻结、其上敌人停止移动；其它地块最多5层只减速50%不冻结（用户反馈「只有雪蔓延到蓝门上的时候冻蓝门」）；积雪只在地面蔓延、不上高台/墙/禁行（用户反馈「积雪不会占高台」）；验证：camp_01 部署位范围13格含8高台→仅5地面格积雪；纯天赋 t=50 满5层 frozenTiles=0（蓝门未被覆盖）、5层格敌人移速50%（dur 10.53 vs 5.26）；S2 开启后扩散至9地面格仍不冻结；蓝门在范围内时 frozenTiles=1（仅蓝门、其余5层格不冻、冻标签1个）；版本标记 v20260815s→t
- 【2026-08-15 干员攻击判定与 Spine 解耦 + 敌人受击闪红】用户反馈「初雪攻击不播放攻击动画、敌人没有受攻效果」；根因一：干员攻击判定整段被 `if (!op.spineAnim) return` 挡住——Spine 模型未加载（或加载慢）时干员完全不进入攻击逻辑，自然没有动画也没有受击反馈；修复：攻击判定与受击反馈不依赖 Spine（模型未加载也照常攻击），动画/朝向仅在模型存在时执行；根因二：模拟器无受击效果；新增受攻闪红——干员攻击命中目标时目标短暂染红 0.18s（约每秒一次模拟攻击间隔，纯视觉不改数据）；官方 gamedata（Kengxxiao）只有 JSON 配置（角色/技能/关卡/敌人表），无视觉特效数据，特效在客户端资源/社区提取（本站在用 isHarryh/Ark-Models 干员模型、serine-qing/MapSimulatorBackend 敌人模型），受击反馈以染色近似；圣聆初雪模型动画清单=Default(0s)/Interact/Move/Relax/Sit/Sleep，攻击动画用 Interact 兜底；验证：无模型时 attacking 正常、命中敌人闪红（redMeshes=1）；有模型时 Relax↔Interact 切换正常；版本标记 v20260815t→u
- 【2026-08-15 修正：S2 积雪扩散逐层推进至最多 20 格】用户反馈「初雪二技能期间积雪会扩散到最大20格」；原实现扩散只走种子格相邻一圈（BFS 不加入新格）即停止，无法随时间推进到 20 格上限；修正：超过 5 层的部分累计为扩散次数（每超一层向外扩一格），BFS 把扩散出的新格继续入队逐层向外推进，最多向外扩散 20 格；验证：camp_01 S2 开启后扩散逐层增长至 5 范围格+20 扩散格=25 格封顶、之后不再增长；纯天赋仍不扩散；版本标记 v20260815u→v
- 【2026-08-15 修正：敌人离开积雪格→该格清空从0层重新叠层】用户反馈「敌人离开哪格积雪就清空哪格层数从0层重新叠层」（对应天赋1「首个敌人离开该地块时积雪消失」）；实现：按格记录「最近清空时间」，每格积雪以 max(部署时间, 该格清空时间) 为基准重新推导层数（天赋每10秒+1、S2每2秒+1）；地面敌人离开积雪格时记录清空（飞行敌人不触发）；重置/重播时清空历史清空事件（时间线重新开始）；验证：camp_01 全新部署逐秒跑到 t=30——敌人走过的 4,0/4,1/4,2 清空为 0 层、4,3 离开后从 0 重新叠回 1 层、未走过的扩散格不受影响；版本标记 v20260815v→w
- 【2026-08-15 修正：敌人离开积雪格清空彻底生效（扩散格不再被 BFS 补回）】用户复测「敌人离开积雪地块后积雪层数未清空」仍存在；根因：二技能扩散出的格子被清空后，下一帧又被 computeSnowState 的扩散 BFS 按“新扩散格”重新补雪（等于没清）；修复：扩散 BFS 跳过 clearAt[k] 已清空的格子（保持清空直到范围内重新叠层）；另修撤离初雪后旧 clearAt 残留——removeOperator 里 SNOW_OP 撤离时清空 state.snow.clearAt 并重置 sig，重新部署从 0 重新叠层；验证：扩散格 4,5/4,6/4,7 清空后连续 8 帧保持 0 层、范围格 4,0/4,2 清空后从 0 重新叠层、无回归；版本标记 v20260815w→x
- 【2026-08-15 敌人路径点坐标三方核对（官方/参考站/PRTS.Map）】按用户要求比对参考站、官方数据库与 PRTS.Map 重核敌人路径点坐标基于场景的位置：官方 route.startPosition/endPosition 的 row 自底向上（0=地图底部），本站 buildRoutePaths 的 toMap={row: rows-1-y, col: x} 翻转后——0-1 WALK 路线 start raw(2,8)→flip(3,8)=tile_start、end raw(3,0)→flip(2,0)=tile_end；TO-EX-2 兔洞出口路线 start flip 命中 tile_rabbithole_out_1、end flip 命中 tile_end(4,6)；36 剿灭关全量 7505 条路线方向性检验无上下颠倒（个别 start/end 落在 road/floor/deepsea/volcano 等均系官方刻意把出生/消失点放地图中部，参考站同样按 isAirborne 处理）；参考站（serine-qing/MapSimulator）TileManager 构造时对地图数组 .reverse()（与本站翻转同一语义），parseEnemyRoutes 以 {x:col, y:row} 比对翻转后 startTiles——与本实现完全一致；PRTS.Map（map.ark-nights.com）bundle 内关卡索引 data_path 直接指向官方 zh_CN/gamedata/levels/obt/... level JSON（三方同源）；结论：路径坐标正确，无需改动代码
- 【2026-08-15 修正：路线折线展平（消除心电图锯齿，对齐参考站）】用户复看两张参考站截图指出「那两个是直线路径，你写的是心电图路径了都」；核查判断机制——三方检查点坐标数据逐条一致（官方 JSON / PRTS.Map API / 参考站 API 全量比对），但官方 FLY 路线检查点本身带 ±1 格锯齿（route 68 的 (3,5),(3,6),(4,7),(3,8)，route 91/97 的 3,4,3,4 交替），本站 3D 透视相机把每个格心折点放大成心电图状（route 68 屏幕投影 y:117→171→171→202→171→276，中间出现回折）；修复：新增 flattenRoutePath——飞行路线在「锚点→更远点」沿线全部可飞时直线直达（route 68/91/97 全部变为 (1,1)→(6,11) 一条直线，route 2 同），地面路线仅剔除「贴直线 ≤0.8 格」的微锯齿、真实拐弯由沿线可通行校验保护（route 43 巡逻往复保留）；罚站 frac 在展平前按完整路径计算再保留（route 68 的 40/20/20 罚站不变）；传送门路线（telePair）不展平保持传送语义；验证：camp_r_33 全部 FLY 路线 ≤2 点、route68 屏幕投影变为 (491,117)→(912,276) 单一直线、route 43/52 地面拐弯保留、camp_01 与 TO-EX-2 回归 0 报错且罚站 12/52 条保留、全量 36 关展平无退化；版本标记 v20260815x→y
- 【2026-08-15 敌人详情卡片可自由拖动】用户要求「让敌人详情卡片可以自由拖动，松手放下」；实现：新增 setupEnemyDetailDrag——#enemyDetail 整卡 pointer 拖动（关闭按钮/查看详情链接等交互元素不启动拖动；4px 阈值防误触；拖动中 userSelect:none 防选文本；位置 clamp 仅保留最小可见区域防止完全拖出视口）；松手放下后把位置记入 state.detailCardPos，之后点其他敌人打开时保持拖放位置、不跳回点击处；验证：camp_r_33 点敌人开卡→拖动 +100/+80 位置实时更新、关闭再打开位置保持、关闭按钮点击正常关闭、<4px 微小移动不触发拖动、0 JS 报错；版本标记 v20260815y→z
- 【2026-08-15 撤销路线展平：恢复官方检查点折线】用户纠正「你得找准路途中的检查点坐标，而不是只判断起点和终点」「把检查点连起来自然是准确的路线了，前提是检查点的坐标准确」；结论：路线就是把官方检查点连起来（飞行=起点→检查点→终点折线，地面=检查点间 A* 寻路），不做任何展平/剔除——中途检查点（含罚站位置）坐标必须与官方逐点一致；已撤销 flattenRoutePath（含 FLY 最远直达与贴直线剔除，删除死代码），route 68/91/97 恢复为官方完整检查点折线（built == officialFlip 逐点一致）；检查点坐标准确性核查：36 剿灭关 + TO-EX-2 + 0-1 全量三方比对（官方 JSON / PRTS.Map API / 参考站 API）逐点一致（参考站仅 motionMode/type 为枚举数字、坐标与时间全同），翻转约定 start/end 落 tile_start/tile_end 已验证；回归：camp_01 189 路线/12 罚站、TO-EX-2 63 路线/52 罚站 0 报错；版本标记 v20260815z→aa
- 【2026-08-15 路线检查点标注罚站时间】用户要求「如果检查点有发展时间，在检查点上标出来」；实现：新增 #routeWaitLabels 覆盖层 + .route-wait-cd 圆形黄色标签，showEnemyRoute 记录 state.highlightRoute，updateRouteWaitLabels 每帧把 WAIT 检查点时间（w.time）经 samplePath(pts, frac) 投影到官方检查点位置显示（如 40s/20s/20s）；hideAllRoutes/换路线/关闭时自动清空；验证：camp_r_33 route 68 高亮→3 标签 40s/20s/20s 精确落在检查点 (3,5)/(3,6)/(4,7) 投影 (634.6,171)/(675,171)/(717.5,202.3)、route 0→10s、关闭后清空、0 JS 报错；版本标记 v20260815aa→ab
...
### 待办
- V0.31（3D 地图页 UI/画面优化）已部署上线试运行，待用户确认后定格；V0.26/V0.27/V0.28 草稿待定
