...
- 【2026-08-15 敌人攻击严格范围实装（修复近战远处误攻）】用户反馈近战仍在攻击很远处的干员；根因：同 id 敌人共享一个 Spine 骨架与画布，updateCombat 中「同 id 任一在攻击 → 整个 id 播 Attack」使远处同 id 敌人也显示攻击动作；修复：每个敌人 id 新增独立 Attack 骨架/画布（仅该 id 有敌人在攻击时渲染），每个敌人按自己的 attacking 状态在移动/攻击贴图间切换，远处敌人保持移动/待机不再误播攻击；敌人索敌判定与显示圆环/命中标记统一走 enemyRangeCells（近战=相邻 4 格、远程=官方 rangeRadius−0.25 有效半径内格），attack/显示/命中三处口径一致；本地验证：camp_01 同 id 史莱姆 t=10 时 6.08 格处敌人 attacking=false 且不显示攻击画面、1 格内敌人正常攻击，0 违例；版本标记 v20260815o→p
