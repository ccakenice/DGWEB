 ...
 *  - 内置 10 分钟缓存 + 风控自动重试, 避免触发 B 站限流(-412/-799/-352)
...
async function getUinfo(mid) {
  ...
  try {
    // x/relation/stat 目前不需要 WBI 签名
    const s = await biliJson('https://api.bilibili.com/x/relation/stat?vmid=' + mid, false);
    if (s.code === 0 && s.data) follower = s.data.follower;
  } catch (e) { /* 容忍失败 */ }
  try {
    // 2024-2025: x/space/acc/info 已废弃，改用 x/space/wbi/acc/info
    const i = await biliJson('https://api.bilibili.com/x/space/wbi/acc/info?mid=' + mid + '&platform=web&web_location=1550101', true);
...
async function getVideos(mid) {
  ...
  for (let pn = 1; pn <= 2; pn++) {
    // 2024-2025: x/space/arc/search 已废弃，改用 x/space/wbi/arc/search
    // dm_img_str / dm_cover_img_str 为 base64 编码的 WebGL/ANGLE 信息，用于风控验证
    const j = await biliJson(
      ...
      '&dm_img_list=[]&dm_img_str=V2ViR0wgMS4wIChPcGVuR0wgRVMgMi4wIENocm9taXVtKQ&dm_cover_img_str=QU5HTEUgKEFwcGxlLCBBTkdMRSBNZXRhbCBSZW5kZXJlcjogQXBwbGUgTTIgUHJvLCBVbnNwZWNpZmllZCBWZXJzaW9uKUdvb2dsZSBJbmMuIChBcHBsZQ&dm_img_inter={"ds":[],"wh":[0,0,0],"of":[0,0,0]}',
      true
    );
    if (j.code !== 0 || !j.data || !j.data.list) {
      if (pn === 1) throw new Error('arc/search rejected, code=' + j.code + ', msg=' + (j.message || ''));
      break;
    }
...
async function biliJson(url, needSign, retryLeft = 1) {
  ...
  if (resp.status === 412 || resp.status === 403) {
    if (retryLeft > 0) { gBuvid = null; gWbi = null; await sleep(800); return biliJson(url, needSign, retryLeft - 1); }
    throw new Error('bilibili risk control, HTTP ' + resp.status);
  }
  const data = await resp.json();
  if (data.code === -799 || data.code === -412 || data.code === -352) {
    if (retryLeft > 0) { gBuvid = null; gWbi = null; await sleep(800); return biliJson(url, needSign, retryLeft - 1); }
    throw new Error('bilibili rate limited, code=' + data.code + ', msg=' + (data.message || ''));
  }
  return data;
}
...
function wbiSign(params, imgKey, subKey) {
  ...
  // 移除 value 中的 "!'()*" 字符
  for (const k in data) {
    data[k] = String(data[k]).replace(/[!'()*]/g, '');
  }

  // 使用 URLSearchParams 生成查询字符串（空格编码为 +，与 Python urllib.parse.urlencode 一致）
